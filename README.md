...
## Google OIDC and web authorization

1. Create Google OAuth web credentials for the administration Vercel project.
2. Add this production callback URI:
   `https://YOUR_WEB_DOMAIN/api/auth/callback/google`.
3. Set `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, and a random `AUTH_SECRET`.
4. Set `AUTH_GOOGLE_ALLOWED_DOMAIN` to the organization's Google Workspace
   domain. Optional exceptions can be comma-separated in `AUTH_ALLOWED_EMAILS`.
5. Create each user in Backbeat Pager before their first login.

Google emails must be verified, domain/allowlist-approved, and match an active
local user. Backbeat Pager does not provision users from Google. Admins can
mutate configuration; responders receive read-only configuration access and
can operate incidents. Every server action independently verifies the session
and role—the Next.js proxy is only an early routing layer.

For local browser tests only, `AUTH_TEST_USER_EMAIL` selects an existing user
without Google. The bypass is ignored when `NODE_ENV=production`.

For preview deployments, use a Neon preview branch and preview-specific Google
callback credentials. Do not point production Slack callbacks at previews.

## Administration console

The web console includes:

- live dashboard metrics, current on-call cards, and recent timeline activity;
- user roles, timezones, Slack mapping, notification preferences, and safe
  activation/deactivation;
- team membership, Slack channel, handoff, and weekly digest configuration;
- service routing-key one-time reveal/rotation, policy/channel/nag/incident
  channel settings;
- multi-step escalation policy editing with user/team/schedule targets;
- schedule rotation/restriction editing, overrides, and a 14-day visual
  calendar generated by the production resolver;
- searchable/paginated incidents with raw alert payloads, complete timeline,
  notifications, Slack projections, and acknowledge/resolve actions;
- centralized team/service Slack channel mapping.

Configuration writes use runtime schemas, database transactions, role checks,
and immutable `AuditLog` entries. Referenced entities are deactivated rather
than unsafely deleted.
