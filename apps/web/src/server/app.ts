...
import { registerReconciliationRoute } from "./routes/reconciliation";
import { registerSlackHttpRoute } from "./routes/slack";
import { registerWorkflowRuntimeRoutes } from "./routes/workflow-runtime";
import type { SlackBoltRuntime } from "./slack/bolt-runtime";
...
export function buildApp(options: BuildAppOptions = {}): FastifyInstance {
  ...
  if (options.slackRuntime) {
    const slackRuntime = options.slackRuntime;
    void app.register((scope, _options, done) => {
      registerSlackHttpRoute(scope, slackRuntime);
      done();
    });
  }
